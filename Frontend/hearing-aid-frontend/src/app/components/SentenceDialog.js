...
import SentenceApis from "../../apis/SentenceApi";

const SentenceDialog = ({ open, onClose, quizType }) => {
	...
	const handleSubmit = async () => {
		...
		try {
			// submit answer
			const response = await SentenceApis.submitAnswer(
				sessionId,
				selectedPayload
			);
			...
			if (response && response.data) {
				...
				const nextResp = await SentenceApis.nextSentence(sessionId);
	...
	const handleShowResults = async () => {
		...
		const resultsResponse = await SentenceApis.getResults(sessionId);
	...
	const sentenceStartSession = async () => {
		try {
			const payload = {
				user_id: "12345",
			...
			setIsSubmitting(true);
			const response = await SentenceApis.startSentences(payload);

			if (response.data && response.data.session_id) {
				const session = response.data.session_id;
				setSessionId(session);
				const firstQuestionResponse = await SentenceApis.nextSentence(session);
			...
			console.error("Error starting sentence session:", error);
		}
	};

	useEffect(() => {
		if (quizType === "sentence" && open) {
			sentenceStartSession();
		}
	}, [quizType, open]);
		...
		const options = [
			{
				...
				label: currentQuestion?.correct?.sentence ?? "",
			},
			{
				key: "similar",
				id: currentQuestion?.similar?.id ?? null,
				label: currentQuestion?.similar?.sentence ?? "",
			},
			{
				key: "other",
				id: null,
				...
				{showResults ? (
					<Grid container spacing={2}>
						<Grid item size={12}>
							...
							<Typography
								...
								gutterBottom
							>
								{correctAnswersCount} / {resultData?.total_sentences_tested}
							</Typography>
							<Grid container spacing={1} sx={{ mb: 1 }}>
								{resultData?.vowels_tested.map((v, i) => (
									<Grid item size={6} key={`sentences-result-${i}`}>
										<Typography
											variant="h5"
											align="center"
											color={
							...
							{resultData?.total_sentences_tested === correctAnswersCount ? (
					...
					<Grid container sx={{ px: 3 }}>
						...
						>
							...
							>
								{questionNumber === 7 ? "Finish" : "Next Question"}
							</Button>
						</Grid>
					</Grid>
				)}
